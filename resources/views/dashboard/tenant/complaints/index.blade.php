...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('dashboard.tenant.complaints.create') }}" class="btn btn-primary"><i class="fa fa-plus me-1"></i> New
            Complaint</a>
    </div>
    ...
    <div class="complaint-card mb-4 p-3">
        <form class="row g-3 align-items-end">
            ...
            <div class="col-md-2">
                <button type="submit" class="btn btn-outline-primary w-100"><i class="fa fa-search me-1"></i>
                    Search</button>
            </div>
        </form>
    </div>
            ...
            <tbody>
                @forelse($complaints as $i => $complaint)
                    <tr>
                        ...
                        <td>
                            <span
                                class="badge bg-{{ strtolower($complaint->status) == 'open' ? 'primary' : 'success' }}">{{ ucfirst($complaint->status) }}</span>
                        </td>
                        <td>{{ $complaint->created_at->format('Y-m-d') }}</td>
                        <td>
                            <a href="{{ route('dashboard.tenant.complaints.show', $complaint->id) }}"
